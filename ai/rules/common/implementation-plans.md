...
- PLAN_AUDIT_MIN_RECHECKS = typecheck | lint | lint:fix | check:circular | test
...
Behavior contracts (validation formats, ownership boundaries, telemetry field semantics) must have one canonical source and explicit cross-links.

Ownership map:
- `docs/system-overview/*` = architecture-level source of truth
- `apps/*/docs/*` = runtime behavior details and operational notes
- `docs/plans/*` = implementation history, gates, and evidence

Mandatory link direction:
1. System-overview docs link to owning app docs and active plan artifacts.
2. App docs link back to system-overview contract docs and relevant plan artifacts.
3. Plan index/phase files link to both system-overview and app docs when behavior contracts change.

Feature/hybrid plan stale-doc checklist:
- Inventory touched docs in all three ownership locations.
- Mark one authoritative contract location per changed behavior.
- Add explicit historical/deprecated markers for superseded docs.
- Verify env keys/examples/telemetry names are consistent across code and docs.
- Include docs verification scans in the phase verification command list.
...
|-----------|-------------|
| Exploration, docs sync, commit-prep | FAST |
...
- Commit-prep only after audit is done
- Move high-risk phases earlier to fail fast

### DB Migration Phases

When a plan phase creates or modifies migration files in `apps/db-migrate/migrations/`, add these verification commands to that phase's checklist (in order):

```
pnpm -w run migrations:eol:check      # CRLF guard — must pass before hash
pnpm -w run migrations:hash           # Rebuild atlas.sum
pnpm -w run migrations:validate       # Atlas integrity check
pnpm --filter @gis-tunnel/db-migrate run migrate:apply:dry  # Dry-run apply
```

Optionally, add a down dry-run when the migration includes a `down.sql` section:

```
pnpm --filter @gis-tunnel/db-migrate run migrate:down:dry
```

These verification commands satisfy `PLAN_AUDIT_MIN_RECHECKS` for the migration phase. Stage both the new `.sql` file and `apps/db-migrate/migrations/atlas.sum` in the commit-prep checklist.

Migration rules: `ai/rules/common/skills/atlas-migration.md`
Privilege/grant guidance: `ai/rules/projects/non-restrict-proxy/skills/database-development.md`
...
- `ai/rules/common/build-and-test.md` — verification commands
